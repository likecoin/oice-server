...
fg_to_bright = {
    "right": """@fgRightToBright key="%(key)s" left="&ch['%(key)s'].xDr" top="&ch['%(key)s'].yDr" storage="%(fg)s" path="&ch['%(key)s'].pathr\"""",
    "left": """@fgLeftToBright key="%(key)s" storage="%(fg)s" left="&ch['%(key)s'].xDl" top="&ch['%(key)s'].yDl" path="&ch['%(key)s'].pathl\""""
}
